...
/// then each friend to send to. Without friends: just the two link buttons.
struct ShareSheetView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Share \(flight.flightNumber)").font(.title2.bold())
            Text("\(flight.departure) → \(flight.arrival) · \(flight.departureDay)").font(.caption).foregroundStyle(.secondary)
            if let friends {
                if friends.isEmpty {
                    HStack {
                        Button("Copy link") { copyLink() }.buttonStyle(.glass).frame(maxWidth: .infinity)
                        Button("Share via…") { Task { shareURL = await mintLink() } }.buttonStyle(.glass).frame(maxWidth: .infinity)
                    }
                } else {
                    Text("Via").font(.caption).foregroundStyle(.secondary).padding(.top, 8)
                    HStack {
                        Circle().fill(.tint).frame(width: 28, height: 28).overlay(Image(systemName: "link").font(.caption).foregroundStyle(.white))
                        Text("Link")
                        Spacer()
                        Button("Copy") { copyLink() }
                        Button("Send…") { Task { shareURL = await mintLink() } }
                    }
                    ForEach(friends) { f in
                        let status = flight.shares.first { $0.person.id == f.person.id }?.status
                        let picked = chosen.contains(f.person.id)
                        HStack {
                            Circle().fill(f.person.tint).frame(width: 28, height: 28)
                                .overlay { if picked { Image(systemName: "checkmark").font(.caption.bold()).foregroundStyle(.white) } else { Text(f.person.givenName.prefix(1).uppercased()).font(.caption.bold()).foregroundStyle(.white) } }
                            Text(f.person.givenName).foregroundStyle(f.person.tint)
                            Spacer()
