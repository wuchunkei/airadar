...
enum FlightReminders {
    ...
    static func schedule(_ flight: Flight) {
        ...
        let who = flight.sharedBy.map { String(localized: "\($0.person.givenName)'s trip") + " · " } ?? ""
        ...
        let terminal = flight.departureTerminal.map { String(localized: " from Terminal \($0)") } ?? ""
        post(.scheduled, at: dep.addingTimeInterval(-16 * 3600), title: String(localized: "\(number) tomorrow · \(route)"),
             body: String(localized: "Departs \(clock.string(from: dep))\(terminal)"))
        ...
        post(.status, at: dep.addingTimeInterval(-LiveActivities.leadIn(for: flight)), title: "\(number) · \(route)",
             body: String(localized: "Departs \(clock.string(from: dep)) · open Airadar to follow it live on your Lock Screen"))
        post(.departure, at: dep.addingTimeInterval(TimeInterval(flight.delayMinutes * 60)), title: "\(number) · \(route)",
             body: flight.delayMinutes > 0 ? String(localized: "Departing now, \(flight.delayMinutes) min late") : String(localized: "Departing now"))
        post(.landed, at: arr.addingTimeInterval(TimeInterval(flight.delayMinutes * 60) + 3600), title: String(localized: "\(number) · landed"),
             body: String(localized: "Arrived \(flight.arrival) · usually \(formatDuration(flight.typicalDurationMinutes ?? flight.durationMinutes))"))
    }
    ...
    static func announceChange(from old: Flight, to new: Flight) {
        ...
        if new.delayMinutes != old.delayMinutes {
            ...
            lines.append(new.delayMinutes > 0 ? String(localized: "Delayed \(new.delayMinutes) min — now departing \(t.clock)") : String(localized: "Back on time — departing \(t.clock)"))
        }
        ...
        if new.boardingStatus != old.boardingStatus, let b = new.currentBoarding, b.announced {
            lines.append(new.departureGate.map { String(localized: "\(b.label) · Gate \($0)") } ?? b.label)
        } else if new.departureGate != old.departureGate, let g = new.departureGate {
            lines.append(String(localized: "Gate \(g)"))
        }
        if new.baggageClaim != old.baggageClaim, let b = new.baggageClaim { lines.append(String(localized: "Baggage belt \(b)")) }
